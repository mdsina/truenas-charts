{{/* Service - Ports */}}
{{/* Call this template:
{{ include "tc.v1.common.lib.service.portsDef" (dict "rootCtx" $rootCtx "objectData" $objectData) -}}
rootCtx: The root context of the chart.
objectData: The object data of the service
*/}}

{{- define "tc.v1.common.lib.service.portsDef" -}}
  {{- $rootCtx := .rootCtx -}}
  {{- $objectData := .objectData -}}

  {{- $tcpProtocols := (list "tcp" "http" "https") -}}
  {{- range $portDef := $objectData.portsDef -}}
    {{- $portValues := $portDef.portValues -}}
    {{- $name := $portDef.name -}}
